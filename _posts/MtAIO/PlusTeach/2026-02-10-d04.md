...
category: Jekyll
layout: post
description: PVE 本地屏幕 btop 监控工具安装配置教程，实现图形化系统监控，展示 CPU、内存、磁盘等硬件信息。文章详细说明了 btop 工具的特性优势、安装依赖配置、主题美化设置以及快捷键使用方法，并提供进程监控、网络流量统计、磁盘 IO 分析等高级功能介绍，帮助用户在 PVE 本地终端获得直观的系统运行状态展示，提升故障排查与性能分析效率。
---
...
还有一个问题如果显示异常，可能是因为系统默认字体不支持 Unicode  
解决方法是在 `/etc/default/console-setup` 中设置` CODESET="Unicode"` 和 `FONTFACE="Terminus"`
<hr />
