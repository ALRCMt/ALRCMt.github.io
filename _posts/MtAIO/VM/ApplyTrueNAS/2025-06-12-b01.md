...
category: Jekyll
layout: post
description: TrueNAS 系统硬盘直通配置教程，指导用户通过 PVE 的 qm set 命令将物理硬盘直通给 TrueNAS 虚拟机，实现 ZFS 存储池的独占访问。文章详细说明了硬盘直通原理、SATA 与 NVMe 硬盘识别方法、qm set 命令参数详解、直通后验证步骤以及数据安全措施，并提供硬盘热插拔注意事项、多硬盘直通方案、ZFS 导入导出操作以及故障排查技巧，帮助用户为 TrueNAS 虚拟机提供可靠的存储硬件支持。
---
